...
TOKEN = "7893705081:AAFf6HIz_R6KqfAcWd2_eI_Zm7dfMC72-xk" # Enter Ur Bot Token
bot = telebot.TeleBot(TOKEN, parse_mode="HTML")

ADMIN_ID = 6321618547 # Enter Ur User ID
print("Bot Started ✅")
...
# giveaways: one gw_id can be posted to multiple channels (same gw_id used)
cursor.execute("""
...
# store per-channel message id so we can edit OLD message when giveaway ends
cursor.execute("""
...
❌ Failed: {failed}""",
        ...
        time.sleep(0.05)  # anti flood safe delay
...
def safe_edit_any(chat_id, message_id, text, reply_markup=None):
    try:
        # First try edit as caption (for photo posts)
...
def handle_template_steps(message):
    ...
    # 🔥 IMPORTANT — এটা function এর ভিতরে থাকতে হবে
...
def use_template(call):
    ...
    # Step 1: Channel selection again
...
def tpl_confirm_channels(call):
    ...
    # ⚠️ must_join overwrite করবি না
    # template থেকে যেটা এসেছে সেটা রাখবি
...
def broadcast_command(message):
    ...
    # If command has text → send as text
...
def confirm_delete(call):
    ...
    # delete giveaway from DB completely
...
"""
    ...
    # এখানে send_photo / send_message যাবে
    posted_names = []
    message_ids = {}
    ...
    # store message ids for editing later
...
# ================= AUTO WINNER SELECTOR (EDIT OLD MESSAGE + DM PRIZES) =================
...
def check_giveaways_loop():
    while True:
        try:
            ...
            for row in rows:
                gw_id, channels_raw, title, description, end_time_str, \
                winners, winner_type, prizes_raw, ended = row
                ...
                # ✅ SAFE TIME CHECK (works for 5m / 1h / 1d / 5d etc)
                remaining_seconds = (end_time - datetime.now()).total_seconds()

                if remaining_seconds > 0:
                    continue

                # mark ended immediately (avoid double run)
...
💝 Enjoy your reward!"""
                ...
                # cleanup participants (keep giveaway record ended=1)
                cursor.execute("DELETE FROM participants WHERE gw_id=?", (gw_id,))
                cursor.execute("DELETE FROM giveaway_messages WHERE gw_id=?", (gw_id,))
                conn.commit()
        except Exception as e:
